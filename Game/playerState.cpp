...
static const float	PLAYER_ENABLE_HP_ATTACK_DIST	= 150.0f;
...
void PlayerState::JudgeChangeHPAttack000( void )
{
	...
	if( pPS4Input->GetTrigger( GetPlayer( )->GetPlayerNo( ) , PS4Controller::DIJ_SQUARE ) )
	{
		...
		//  ブレイブ攻撃に変更
		GetPlayer( )->ChangeState( GetPlayer( )->GetPlayerState( Player::STATE::ATTACK_BRAVE000 ) );
	}
...
void PlayerState::JudgeChangeBraveAttack000( void )
{
	...
	if( GetPlayer( )->GetTargetDistance( ) <= PLAYER_ENABLE_HP_ATTACK_DIST )
	{ 
		if( pKeyboard->GetKeyboardTrigger( nKey ) )
		{
			//  ブレイブ攻撃000状態に変更
