...
void Scene2DL::SetVertex( void )
{
	...
	if( m_pVtxBuff != NULL )
	{
		...
		pVtx[ 1 ].tex = D3DXVECTOR2( m_posUV.x + m_endUV.x / m_divideUV.x , m_posUV.y );
		pVtx[ 2 ].tex = D3DXVECTOR2( m_posUV.x , m_posUV.y + m_endUV.y / m_divideUV.y );
		pVtx[ 3 ].tex = D3DXVECTOR2( m_posUV.x + m_endUV.x / m_divideUV.x , m_posUV.y + m_endUV.y / m_divideUV.y );
